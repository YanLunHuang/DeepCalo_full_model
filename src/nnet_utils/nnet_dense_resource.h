...
template<class data_T, class weight_T,class extend_T, class ret_T>
...
product_dense2(data_T a, weight_T w){
    // 'Normal' product
...
void dense_large_rf_leq_nin(
	...
	for (int im = 0; im < block_factor; im++) {
	    ...
	    tmpmult[im] = product_dense2<data_T, typename CONFIG_T::weight_t, typename CONFIG_T::extend_t, typename CONFIG_T::accum_t>(data[in_index], weights[w_index]);
	    in_index += rufactor;
	    if (in_index >= nin) {
             in_index = ir;
            }
     ...
     for(int i_in = 0; i_in < CONFIG_T::n_in; i_in++) {
        ...
        for (int iacc = 0; iacc < reuse; iacc++) {
          ...
          for (int iacc2 = 0; iacc2 < block_factor; iacc2++) {
            ...
            acc[iacc2][iacc] += product_dense2<data_T, typename CONFIG_T::weight_t, typename CONFIG_T::extend_t, typename CONFIG_T::accum_t>(tmpdata, weights[w_index]);
          }
      }
     }
     ...
     Loop2_1: for(int i_in = 0; i_in < CONFIG_T::n_in; i_in++) {
        ...
        Loop2_2: for (int iacc = 0 ; iacc < reuse; iacc++) {
          ...
          Loop2_3: for (int iacc2 = 0; iacc2 < block_factor; iacc2++) {
            ...
            tmpt[iacc2] = product_dense2<data_T, typename CONFIG_T::weight_t, typename CONFIG_T::extend_t, typename CONFIG_T::accum_t>(tmpdata[i_in], weights[i_in][w_index]);
			//std::cout <<w_index<<" ";
